...
import RefundDialog from '@/components/payments/RefundDialog.vue';
import TransactionStatusTag from '@/components/payments/TransactionStatusTag.vue';
...
function openRefund(item: TransactionItem): void {
    selected.value = item;
...
<template>
    <div>
        <DataTable :value="transactions" size="small">
            ...
            <Column
                v-if="showTreatment"
                :header="t('balance.columns.treatment')"
            >
                <template #body="{ data }: { data: TransactionItem }">
                    <Link
            ...
            <Column :header="t('balance.columns.status')">
                <template #body="{ data }: { data: TransactionItem }">
            ...
            <Column
                v-if="canRefund"
                :header="t('balance.columns.actions')"
                headerClass="text-right"
                bodyClass="w-16 text-right"
            >
                <template #body="{ data }: { data: TransactionItem }">
                    <Button
                        v-if="isRefundable(data)"
                        type="button"
                        severity="secondary"
                        text
                        rounded
                        size="small"
                        :aria-label="t('refund.button')"
                        v-tooltip.top="t('refund.button')"
