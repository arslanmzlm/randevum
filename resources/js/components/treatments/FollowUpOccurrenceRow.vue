...
const props = defineProps<{
    /** Position of this row in `form.follow_up.occurrences` — also its label number. */
    index: number;
    doctorId: number;
    typeOptions: Array<{ label: string; value: number; color: string }>;
    /** Removal is blocked below the package minimum of 2 rows. */
    removable: boolean;
}>();
...
const occurrence = computed(() => form.follow_up.occurrences[props.index]);
...
const rowError = computed<string | undefined>(
    () =>
        fieldError(`follow_up.occurrences.${props.index}.starts_at`) ??
        fieldError(`follow_up.occurrences.${props.index}`),
);

const typeError = computed<string | undefined>(() =>
    fieldError(`follow_up.occurrences.${props.index}.appointment_type_id`),
);

const durationError = computed<string | undefined>(() =>
    fieldError(`follow_up.occurrences.${props.index}.duration_minutes`),
);
...
// never re-checks the others. Advisory — the server runs the full 3-layer check per occurrence.
    ...
    >
        <div class="flex items-center justify-between gap-2">
            <span class="text-sm font-medium text-surface-700">
                {{
                    t('treatment.follow_up.occurrence_label', { n: index + 1 })
                }}
            </span>
            <Button
                v-if="removable"
                ...
                severity="danger"
                size="small"
                :aria-label="t('treatment.follow_up.occurrence_remove')"
                @click="emit('remove')"
            >
                <template #icon>
