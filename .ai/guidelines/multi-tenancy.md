...
- `clinics.tenant_id` is a NOT NULL FK. Every operational table (`patients`, `appointments`, `treatments`, `cases`, `services`, `transactions`, `status_logs`, ...) carries a NOT NULL `clinic_id`. EXCEPTIONS (nullable `clinic_id`, platform-level rows with no clinic): `sms_logs` (OTP login SMS), and `legal_documents` / `consents` (signup Terms/Privacy/DPA). These still use `BelongsToClinic`; platform-level rows are read with `withoutGlobalScopes()` + an explicit `whereNull('clinic_id')`.
