...
- MVP SMS impl is `NetgsmSmsProvider`. Provide a `NullSmsProvider` for test/local.
- Send ALL SMS through `SendSmsJob`, queued on the `sms` queue. Never call a provider synchronously.
- Log every send to `sms_logs` (polymorphic `loggable`): status `Queued`→`Sent`/`Failed`; if a clinic disabled SMS, write status `Skipped` but still log it. Snapshot `phone` and `body` onto the log row.
