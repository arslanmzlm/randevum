...
- Separate workloads via Horizon supervisors without splitting code: `default`, `sms`, `media`, `high` (payment/appointment-critical). Keep SMS on its own queue so it can scale independently.
